...
namespace SmashBros
{
	...
	void Player::setHitbox(float x1, float y1, int w, int h)
	{
		...
		hitbox->x = x + x1;
		hitbox->y = y + y1;
		hitbox->width = (int)(w*Scale);
		hitbox->height = (int)(h*Scale);
	}
	...
	void Player::checkAttacks(const String&ignored)
	{
		...
		if(attacksHolder>-1 || hurt==2 || !checkIfAble() || !itemDo || grabbing || heldByPlayer)
		{
			canDo=false;
		}
	...
	boolean Player::checkEnemyHang()
	{
		for(int i=0; i<=Global::possPlayers; i++) //TODO loop characters
		{
			...
			if(playr!=null && playr!=this) //TODO remove that last check if it messes things up
			{
				switch(playerdir)
				{
					case LEFT:
	...
	void Player::Update(long gameTime)
	{
		...
		if(holdingPlayer && grabbedPlayer != NULL)
		{
			changeTwoSidedAnimation("holding", NO_CHANGE);
			grabbedPlayer->x = x + ((itemOffsetX + grabbedPlayer->hitbox->width) * getPlayerDirMult());
			grabbedPlayer->y = y + itemOffsetY;
	...
	void Player::causeHurt(Player*collide, byte dir, long time)
	{
	...
	void Player::grabAttackSwing()
	{
		...
		causeHurtLaunch(grabbedPlayer, (int)getPlayerDirMult(),2.8f,1.4f, -1,2.8f,1.4f);
		causeHurt(grabbedPlayer, getOppPlayerDir(), 1);
	}
