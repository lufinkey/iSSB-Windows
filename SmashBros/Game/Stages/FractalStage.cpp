...
namespace SmashBros
{
	...
	{
		nextSpawnTime = (long)(random()*14000);
	}
	...
	void FractalStage::RandomStuff::Update(long gameTime)
	{
		if(Global::getWorldTime() >= nextSpawnTime)
		{
			...
			else
			{
				...
				nextSpawnTime = Global::getWorldTime() + (long)(random()*5000);
			}
		}
	...
	FractalStage::RandomThing::RandomThing() : GameElement((float)(random()*900), (float)(random()*600))
	{
		newScale = 1;
		scaleIncr = 1;
		isDead = false;
		
		int creature = (int)(random()*2);
		...
		float moveX = (float)(random()*900);
		float moveY = (float)(random()*600);
		float speed = 2;
	...
	FractalStage::RandomThing::RandomThing(boolean create) : GameElement((float)(random()*900), (float)(random()*600))
	{
		newScale = 1;
		scaleIncr = 1;
		isDead = false;
	...
	void FractalStage::Fractal::Draw(Graphics2D&g, long gameTime)
	{
		...
		color1 = Color((unsigned char)(random()*255),(unsigned char)(random()*255),(unsigned char)(random()*255));
		color2 = Color((unsigned char)(random()*255),(unsigned char)(random()*255),(unsigned char)(random()*255));
		...
		g.rotate((float)(random()*360), 450, 300);
	...
	{
		Color c1 = Color((unsigned char)(random()*255),(unsigned char)(random()*255),(unsigned char)(random()*255));
		Color c2 = Color((unsigned char)(random()*255),(unsigned char)(random()*255),(unsigned char)(random()*255));
		while(c1.equals(c2))
		{
			c2 = Color((unsigned char)(random()*255),(unsigned char)(random()*255),(unsigned char)(random()*255));
		}
