...
namespace SmashBros
{
	...
	void Mario::onAnimationFinish(const String&n)
	{
		...
		else if(n.equals("finalsmash_begin_left") || n.equals("finalsmash_begin_right"))
		{
			...
			createProjectile(new FinalSmashFire(getPlayerNo(), x+(getPlayerDirMult()*60), y-40));
		}
	...
	void Mario::attackB()
	{
		if(!bUp)
		{
			...
			{
				AttackTemplates::singleProjectile(this, 10,0, new Fireball(this->getPlayerNo(),x,y));
	...
	void Mario::attackUpB()
	{
		if(!bUp)
		{
			AttackTemplates::rehitUpB(this, 12, 3.8, 5.5f, 5.5f);
			if(isOnGround())
			{
				xvelocity = getPlayerDirMult()*0.8f;
			}
			else
			{
				xvelocity = getPlayerDirMult()*2.1f;
			}
		}
	}
