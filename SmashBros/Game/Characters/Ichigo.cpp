...
namespace SmashBros
{
	...
	void Ichigo::onAnimationFinish(const String&n)
	{
		...
		else if(n.equals("special_prep_down_left") || n.equals("special_prep_down_right"))
		{
			if(isOnGround())
			{
				AttackTemplates::singleProjectile(this, 14, 1.6, new GroundStab(getPlayerNo(),x,y + 2));
	...
	void Ichigo::whilePlayerColliding(Player*collide, byte dir)
	{
		if(attacksHolder==11)
		{
			collide->x += getPlayerDirMult() * (std::abs(xVel)+1);
		}
	}
	...
	void Ichigo::attackB()
	{
		if(!bUp)
		{
			...
			{
				AttackTemplates::chargeB(this, 30, 100, 300);
				attacksHolder=10;
				attacksPriority=0;
				charging = 1;
	...
	{
		addAttackInfo(DIR_LEFT, 18, LEFT, 10, 90, -1,5,8, -1,5,8);
		addAttackInfo(DIR_RIGHT,18,RIGHT, 10, 90,  1,5,8, -1,5,8);
