...
namespace SmashBros
{
	...
	void Sonic::Load()
	{
		...
		addTwoSidedAnimation("special_prep", "special_prep.png", 8, 5, 1);
	...
	void Sonic::Update(long gameTime)
	{
		...
		if(attacksHolder==12)
		{
			...
			else
			{
				...
				{
					turning=false;
					xVel = getPlayerDirMult()*rollSpeed;
					attacksHolder=12;
					moveLeft=0;
					moveRight=0;
					changeTwoSidedAnimation("roll", NO_CHANGE);
		...
		if(attacksHolder==14)
		{
			...
			else
			{
				...
				{
					turning=false;
					xVel = getPlayerDirMult()*rocketSpeed;
					attacksHolder=14;
					moveLeft=0;
					moveRight=0;
					changeTwoSidedAnimation("roll", NO_CHANGE);
				...
				else
				{
					...
					if(moving)
					{
						...
						else if(animName.equals((String)"finalsmash_turn" + suffix))
						{
							...
							if(animpos<=0)
							{
								x += getPlayerDirMult()*sonicSpeedRun*0.25f;
							}
							else if(animpos<=1)
							{
								x += getPlayerDirMult()*sonicSpeedRun*0.5f;
							}
							else
							{
								x += getPlayerDirMult()*sonicSpeedRun*0.75f;
							}
						}
						else
						{
							x += getPlayerDirMult()*sonicSpeedRun;
	...
	void Sonic::onAnimationFinish(const String&n)
	{
		...
		{
			winding = false;
			AttackTemplates::normalSmash(this,15,3.11,STEP_GO,1,1500,14,20);
		}
		...
		{
			animFinish();
			AttackTemplates::rollAttack(this, 12, 3.09, rollSpeed, 1600);
			xvelocity=0;
			yvelocity=-5;
		}
		else
	...
	void Sonic::onPlayerHit(Player*collide, byte dir)
	{
		switch(dir)
		{
			...
			if(attacksPriority==7)
			{
				...
				collide->y-=5;
				causeHurtLaunch(collide, (int)getPlayerDirMult(),0,0, multY,abs(distY),6);
				causeHurt(collide, getOppPlayerDir(), 400);
			}
			break;
		...
		switch(dir)
		{
			...
			if(attacksHolder==16)
			{
				...
				switch(hitAmount)
				{
					...
					collide->yvelocity = 1;
					collide->x = x + (getPlayerDirMult()*10);
					causeHurt(collide, getOppPlayerDir(), 300);
					hitAmount+=1;
					break;
	...
	Sonic::Spring::~Spring()
	{
		if(owner!=NULL)
		{
			if(owner->lastSpring==this)
			{
				owner->lastSpring = NULL;
			}
		}
	}
