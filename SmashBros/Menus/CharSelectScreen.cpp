...
namespace SmashBros
{
	...
	CharSelectScreen::CharSelectScreen(String name) : Screen(name)
	{
		canGrabCoin = false;
		selectedCoin = 0;
		cardTypes = null;
	...
	void CharSelectScreen::Update(long gameTime)
	{
		...
		for(int i=0; i<charCoins.size(); i++)
		{
			for(int j=0; j<charIconPoints.size(); j++)
			{
				...
				{
					Global::selectedChar[i]=charIcons.get(j)->num;
	...
	void CharSelectScreen::selectRandomCharacter(int coin)
	{
		int select = (int)(random()*charIcons.size());
		int index = coin-1;
	...
	{
		if(!teamFlag->mouseOver() && !teamFlag->isClicked() && !teamFlag->wasClicked())
		{
			switch(type)
			{
				default:
