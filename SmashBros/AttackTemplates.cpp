...
namespace SmashBros
{
	...
	void AttackTemplates::causeHurt(Player*playr, Player*collide, byte dir, long time)
	{
		if(collide->deflectState)
		{
			byte d = Player::LEFT;
	...
	void AttackTemplates::dropAirDownA(Player*playr, int type, int aNo, double aP, float speed, float speedX)
	{
		playr->hurt = 0;
		playr->up=false;
		playr->jumping=false;
	...
	void AttackTemplates::homingB(Player*playr, int aNo, double aP, float speed)
	{
		...
		{
			speedX= playr->getPlayerDirMult()*0.7f*speed;
			speedY=0.7f*speed;
		}
	...
	void AttackTemplates::moveSideB(Player*playr, int aNo, double aP, float xDist)
	{
		...
		playr->hurt = 0;
		playr->changeTwoSidedAnimation("special_attack_side", FORWARD);
		playr->x += playr->getPlayerDirMult()*xDist;
		playr->attacksHolder=aNo;
		playr->attacksPriority=aP;
	}
