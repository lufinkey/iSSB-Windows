...
namespace GameEngine
{
	...
	Animation::Animation(const String&name,int speed)
	{
		rows = 0;
		cols = 0;
		sequence = NULL;
	...
	Animation::Animation(const String&name,int speed,int rows,int cols) //(animType 1)
	{
		sequence = NULL;
		names = ArrayList<String>();
		recentFrame = 0;
	...
	void Animation::addFrame(const String&fImage) //adds a frame to the animation
	{
		...
		switch(animType)
		{
			...
			{
				Console::WriteLine((String)"Error: Attempt to add image " + fImage + (String)" to animation failed!");
			}
			break;
	...
	void Animation::drawFrame(Actor*a,Graphics2D& graphics,int fNum, float x1, float y1, double scale, bool relativeToScreen)
	{
		...
		if(a!=NULL)
		{
			...
			alpha = (unsigned char)((1 - a->getAlpha())*g.getAlpha());
			color = a->getColor();
			if(a->mirrored)
			{
				if(mirrored)
	...
	void Animation::drawFrame(Actor*a,Graphics2D& graphics,int fNum, float x1, float y1, float x2, float y2)
	{
		...
		{
			g.setAlpha(alpha);
			g.rotate((float)rotation ,x1 + ((x2-x1)/2), y1 + ((y2-y1)/2));
		}
		g.setImageMask(color);
