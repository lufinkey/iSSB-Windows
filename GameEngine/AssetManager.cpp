...
namespace GameEngine
{
	...
	BufferedImage* AssetManager::getImage(const String&imgName)
	{
		...
		Console::WriteLine((String)"Error: image " + imgName + (String)" does not exist!");
		return NULL;
	}
	...
	bool AssetManager::loadFont(const String&fontName, int style, int size)
	{
		if(!alreadyStoredFont(fontName))
		{
			Console::WriteLine((String)"Loading font " + fontName);
			sf::Font*derive = new sf::Font();
			bool success = derive->loadFromFile(fontName);
			if(success)
			{
				Fonts.add(new Font(derive, style, size));
	...
	Font*AssetManager::getFont(const String&fontName, int style, int size)
	{
		int total = FontNames.size();
		for(int i=0; i<total; i++)
		{
